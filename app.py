...
import re
from trendingmovies import copyoftrending
...
def display_popular_movies():
    try:
        ...
        for i in range(0,20):
            trending_movie_poster.append(fetch_poster(popular_movies_response_data["results"][i]["id"])) ## for id
...
if st.session_state.page == "Movies":
    ...
    if st.button('Show Recommendation'):
        try:
            ...
            st.text("Internet speed problem")
    ...
    if st.button("Search"):
        try:
            ...
            st.header("Error 404 , I don't have money to pay api bill.")

elif st.session_state.page == "Trending":
    st.session_state.page = display_popular_movies()

# things to ,
# 1.Add trailer to link or playable trailer , when are recommended
# 2. where the user can watch it  {ott platform names}
